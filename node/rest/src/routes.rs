...
impl<N: Network, C: ConsensusStorage<N>, R: Routing<N>> Rest<N, C, R> {
    ...
    pub(crate) async fn connect_peer(
        ...
        if let Ok(ip) = peer_ip.parse::<SocketAddr>(){
            if !rest.routing.router().is_connected(&ip) {
                rest.routing.router().connect(ip);
            }

            Ok(ErasedJson::pretty("ok"))
        }else{
            Ok(ErasedJson::pretty("incorrect ip"))
        }
