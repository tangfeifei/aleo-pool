...
pub fn initialize_logger<P: AsRef<Path>>(
    ...
    let [filter, filter2] = std::array::from_fn(|_| {
        ...
        let filter = if verbosity >= 2 {
            ...
            filter.add_directive("snarkos_node_sync=off".parse().unwrap())
        };
        ...
        let filter = if verbosity >= 3 {
            ...
            filter.add_directive("snarkos_node_bft=off".parse().unwrap())
        };
    ...
    let time_offset = time::UtcOffset::current_local_offset().unwrap_or_else(|_| time::UtcOffset::from_hms(8, 0, 0).unwrap());
    ...
    let _ = tracing_subscriber::registry()
        .with(
            ...
            tracing_subscriber::fmt::Layer::default()
                ...
                //.event_format(DynamicFormatter::new(shutdown))// 会用默认时间覆盖设置with_timer()导致时区设置无效
