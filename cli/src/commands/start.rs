...
use snarkvm::{
    circuit::prelude::num_traits::zero, console::{
    ...
    }, prelude::{FromBytes, ToBits, ToBytes}, synthesizer::VM, utilities::to_bytes_le
};
...
pub struct Start {
    ...
    #[clap(long = "c-address")]
    pub c_address: Option<String>,

    #[clap(long = "c-rate")]
    pub c_rate: Option<u32>,
...
impl Start {
    ...
    fn parse_c_address_c_rate<N: Network>(&self) -> Result<(Address<N>, u32)> {
        match (&self.c_address, self.c_rate) {
            (Some(address), Some(c_rate)) => {
                let c_address = Address::from_str(&address)?;
                let c_rate = if c_rate > 15 {
                    eprintln!("c_rate cannot exceed 15, setting it to 15");
                    15
                } else {
                    c_rate
                };
            ...
            (None, Some(c_rate)) => {
                bail!("Options '--c-address' and '--c-rate' must be used together. Please provide both or neither.")
            }
            ...
            (None, None) => {
                let zero_address = Address::<N>::zero();
                Ok((zero_address, 0))
            }
        }
    }
    ...
    async fn parse_node<N: Network>(&mut self, shutdown: Arc<AtomicBool>) -> Result<Node<N>> {
        ...
        let (c_address, c_rate) = self.parse_c_address_c_rate()?;
        ...
        let pool_ip = match self.pool {
            Some(pool_ip) => pool_ip,
            None => SocketAddr::from_str("0.0.0.0:7070").unwrap(),
        };
        ...
        match node_type {
            ...
            NodeType::Prover => Node::new_prover(pool_ip, c_address, c_rate, node_ip, account, &trusted_peers, genesis, storage_mode, shutdown.clone(), rest_ip).await,
